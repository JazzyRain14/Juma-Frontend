    ...
    <>
      <div className='border relative justify-center items-center h-[300px]'>
        <img
          ...
          src={ComponentBg} alt="" />
        <h1
          className='text-[5rem] flex justify-center items-center text-white w-full h-full text-center font-semibold tracking-widest'>Bakery Items</h1>
      </div>

      <div className='px-4'>
        <section className='py-4'>
          {projectImport.map((card) => (
            card.Product4.map((allSnacks, index) => (
              <div key={index}>
